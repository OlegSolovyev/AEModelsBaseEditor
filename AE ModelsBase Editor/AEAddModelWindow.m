...
- (IBAction)addModelButtonPressed:(id)sender {
    ...
    if([self.brandComboBox.stringValue isEqualToString:@""]){
        ...
        NSString *result = [NSString stringWithFormat:@"\nModel\nIndex:\n%d\nBrand:\n%@\nName:\n%@\nMin Year:\n%d\nMax Year:\n%d\nDiesel:\n%d;%d\nGas:\n%d;%d\nInjector:\n%d;%d\nCarburetor:\n%d;%d\nManual Transmission:\n%d;%d\nDSG:\n%d;%d\nHydro:\n%d;%d\nVariator:\n%d;%d\n", index,self.brandComboBox.stringValue, [[self.modelComboBox.stringValue componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]] componentsJoinedByString:@" "], self.minYearField.intValue, self.maxYearField.intValue, (self.dieselBox.state == NSOnState) ? 1 : 0, self.dieselYearField.intValue, (self.gasBox.state == NSOnState) ? 1 : 0, self.gasYearField.intValue, (self.injectorBox.state == NSOnState) ? 1 : 0, self.injectorYearField.intValue, (self.carburetorBox.state == NSOnState) ? 1 : 0, self.carburetorYearField.intValue, (self.manualBox.state == NSOnState) ? 1 : 0, self.manualYearField.intValue, (self.DSGBox.state == NSOnState && self.automaticBox.state == NSOnState) ? 1 : 0, self.DSGYearField.intValue, (self.hydroBox.state == NSOnState && self.automaticBox.state == NSOnState) ? 1 : 0, self.hydroYearField.intValue, (self.variatorBox.state == NSOnState && self.automaticBox.state == NSOnState) ? 1 : 0, self.variatorYearField.intValue];
